...
fn load_properties(conn: &rusqlite::Connection, supervisor_id: i64) -> Result<Vec<Property>, String> {
    let mut stmt = conn
        .prepare(
...
pub fn save_company(company: Company) -> Result<OperationResult<Company>, String> {
    ...
    if company.company_id < 1000 || company.company_id > 9999 {
        return Ok(OperationResult::err("Company ID must be between 1000 and 9999."));
    }
...
fn save_supervisors(conn: &rusqlite::Connection, company_id: i64, supervisors: &[Supervisor]) -> Result<(), String> {
...
fn save_properties(conn: &rusqlite::Connection, supervisor_id: i64, properties: &[Property]) -> Result<(), String> {
    for p in properties {
...
#[tauri::command]
pub fn get_company_property_addresses(company_name: String) -> Result<Vec<(String, Property)>, String> {
    with_conn(|conn| {
        let mut out = Vec::new();
        let mut stmt = conn
            .prepare(
            ...
            .map_err(|e| e.to_string())?;
        let rows = stmt.query_map([&company_name], |r| {
    ...
    with_conn(|conn| {
        if contractor.id == 0 {
            let dup: Option<i64> = conn
                ...
                .map_err(|e| e.to_string())?;
            if dup.is_some() {
                return Ok(OperationResult::err("A contractor with this name already exists."));
            }
            conn.execute(
...
#[tauri::command]
pub fn apply_receivable_payments(invoices: Vec<Invoice>) -> Result<OperationResult<String>, String> {
    for inv in &invoices {
        if inv.amount_paid1 + inv.amount_paid2 > inv.amount_cost {
            return Ok(OperationResult::err(
    ...
    with_conn(|conn| {
        ...
        Ok(OperationResult {
            ...
            data: Some(format!("Updated {} invoice(s). {} fully paid.", updated, fully_paid)),
        })
    })
}
...
pub fn import_companies_csv(rows: Vec<CsvCompanyRow>) -> Result<OperationResult<i32>, String> {
    with_conn(|conn| {
        ...
        Ok(OperationResult::ok_msg(count, &format!("Imported {} companies.", count)))
    })
}
...
pub fn import_properties_csv(rows: Vec<CsvPropertyRow>) -> Result<OperationResult<i32>, String> {
    with_conn(|conn| {
        ...
        Ok(OperationResult::ok_msg(count, &format!("Imported {} properties.", count)))
    })
}
...
pub fn import_sales_csv(rows: Vec<CsvSalesRow>) -> Result<OperationResult<i32>, String> {
    with_conn(|conn| {
        ...
        Ok(OperationResult::ok_msg(count, &format!("Imported {} sales rows.", count)))
    })
}
